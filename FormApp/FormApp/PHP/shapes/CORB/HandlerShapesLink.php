<?php
	abstract class HandlerShapesLink extends HandlerShapes
	{
		...
		public function createShapeSpec($shapeAsArray)
		{
			...
			else
			{
				...
				if ($this->successor!=null)
					return $this->successor->createShapeSpec($shapeAsArray);
				else
					return null;
			}
		}
		...
		{
			var_dump($this);
			echo("Dans PUSHSHAPE : <br/>");
			var_dump($this->shapeType);
			echo("Maillon actuel : ".get_class($this)."<br/>");
			$currentLinkShape = $this->pushShape2($shape);

			if ($currentLinkShape != null)
				return $shape;
			else
			{
				echo("Prochain maillon : ".get_class($this->successor)."<br/>");
				if ($this->successor!=null)
		...
		public function pullShape1($shape)
		{
			$currentLinkShape = $this->pullShape2($shape);
			...
			{
				if ($this->successor!=null)
					return $this->successor->pullShape1($shape);
				else
					return null;
			}
		}
	    ...
	    {
	    	return $this->pushShape2($shape);
	    }

	    abstract public function pushShape2(&$shape);
	    abstract public function createShape2();
	    abstract public function pullShape2($shape);
	}
?>